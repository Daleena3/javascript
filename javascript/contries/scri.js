var countries=[
  ...
  var names=countries.map(u=>u.name)
  var sbox=document.querySelector("#id_select")
  ...
  for ( let name of names
    ...
    function fetchCOUNTRY(){
        let cname=id_select.value
        let data=countries.find(u=>u.name==cname)
        populateresult(data)
    }
    function populateresult(data){
        ...
        <div class="card-body">
          <h5 class="card-title">    <h3><b>C</b> oun<b>T</b> <b>r</b>y</h3>
          </h5>
        </div>
        <ul class="list-group list-group-flush">
          <li class="list-group-item">contry name${data.name}</li>
